...
Setiap kali transaksi biasa diedit (`editTransaction()` lalu simpan
ulang), perubahannya sekarang dicatat — siapa yang mengedit, kapan, dan
field apa saja yang berubah (dari → ke). Bisa dilihat lewat tombol
"🕘 Riwayat" di Riwayat (khusus tampilan pemilik, sebaris dengan tombol
Edit/Hapus). Butuh tabel baru — jalankan sekali di Supabase SQL Editor:

```sql
create table if not exists transaction_edit_log (
  id bigint generated by default as identity primary key,
...
alter table transaction_edit_log enable row level security;

create policy "Pemilik & kasir toko bisa akses riwayat edit toko"
on transaction_edit_log
for all
using (
  owner_id = auth.uid()
  or owner_id in (
...
Aman dijalankan kapan saja — kalau tabelnya belum ada, `logTransactionEdit()`
gagal diam-diam (edit transaksinya sendiri tetap berhasil normal, cuma
...
`diffTransactionFields()`) — bukan snapshot penuh tiap kali disimpan,
supaya riwayatnya ringkas dan gampang dibaca.

Scoped ke transaksi biasa dulu (bukan Paket Bulanan/Tempo, Pengeluaran,
atau Katalog) karena itu catatan finansial yang paling sering diedit dan
paling perlu akuntabilitas — bisa diperluas ke entitas lain kalau
dibutuhkan.
